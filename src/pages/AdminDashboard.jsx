...
export default function AdminDashboard() {
  ...
  async function handleDelete(u) {
    ...
    if (USE_API) {
      try {
        const res = await fetch(`http://localhost:4000/users/${encodeURIComponent(u.username)}`, { method: 'DELETE' })
        if (!res.ok) throw new Error('failed')
  ...
  async function handleSaveEdit(u) {
    ...
    if (USE_API) {
      try {
        const res = await fetch(`http://localhost:4000/users/${encodeURIComponent(u.username)}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
    ...
    const next = users.map(x => x.username === u.username ? { ...x, password: editingPassword } : x)
    saveUsers(next)
    setEditing(null)
    setEditingPassword('')
            ...
            {users.map(u => (
              <li key={u.username}>
                <div style={{display:'flex',gap:12,alignItems:'center'}}>
                  ...
                  {editing === u.username ? (
                    <>
                      <input placeholder="Nova senha" value={editingPassword} onChange={e => setEditingPassword(e.target.value)} />
                      <button onClick={() => handleSaveEdit(u)}>Salvar</button>
                      <button onClick={() => { setEditing(null); setEditingPassword('') }}>Cancelar</button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => { setEditing(u.username); setEditingPassword('') }}>Editar senha</button>
